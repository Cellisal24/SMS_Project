        ...
        <div class="container-fluid px-3 px-lg-4 py-4">
          <div class="page-heading d-flex flex-column flex-lg-row align-items-start align-items-lg-center justify-content-between gap-3 mb-4">
            <div>
              ...
              <h1 class="h2 fw-bold mb-2">កែប្រែកម្រិតថ្នាក់</h1>
              <p class="text-muted mb-0">Update the details of an existing grade level or academic stage in the system. This page now uses the full admin content width for a better form experience.</p>
            </div>
            ...
            <div class="col-12 col-xl-10">
              <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
                ...
                <div class="card-body p-4 p-md-5 bg-white">
                  ...
                  <form action="{{ route('grade-levels.update', $gradeLevel) }}" method="POST" class="row g-4">
                    @csrf
                    @method('PUT')

                    <div class="col-12">
                      <label for="level_name" class="form-label fw-semibold">ឈ្មោះកម្រិតថ្នាក់ (Grade Level Name) <span class="text-danger">*</span></label>
                      <input type="text" id="level_name" name="level_name" value="{{ old('level_name', $gradeLevel->level_name) }}" placeholder="ឧទាហរណ៍៖ Grade 10, Year 1, ថ្នាក់ទី១២" required class="form-control form-control-lg shadow-sm border-1 border-gray-200">
                    </div>
                    <div class="col-md-6">
                      ...
                      <select id="stage" name="stage" required class="form-select form-select-lg shadow-sm border-1 border-gray-200">
                        ...
                        <option value="Primary School" {{ old('stage', $gradeLevel->stage) == 'Primary School' ? 'selected' : '' }}>Primary School (បឋមសិក្សា)</option>
                        <option value="Secondary School" {{ old('stage', $gradeLevel->stage) == 'Secondary School' ? 'selected' : '' }}>Secondary School (អនុវិទ្យាល័យសិក្សា)</option>
                        <option value="High School" {{ old('stage', $gradeLevel->stage) == 'High School' ? 'selected' : '' }}>High School (វិទ្យាល័យសិក្សា)</option>
                        <option value="Foundation Year" {{ old('stage', $gradeLevel->stage) == 'Foundation Year' ? 'selected' : '' }}>Foundation Year (ឆ្នាំសិក្សាមូលដ្ឋាន)</option>
                        <option value="Bachelor" {{ old('stage', $gradeLevel->stage) == 'Bachelor' ? 'selected' : '' }}>Bachelor Degree (បរិញ្ញាបត្រ)</option>
                      </select>
                    </div>

                    <div class="col-md-6">
                      <label for="sort_order" class="form-label fw-semibold">លេខរៀបលំដាប់ (Sort Order) <span class="text-danger">*</span></label>
                      <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $gradeLevel->sort_order) }}" min="1" required placeholder="ឧទាហរណ៍៖ លេខ ១ សម្រាប់ថ្នាក់ទាបជាងគេ" class="form-control form-control-lg shadow-sm border-1 border-gray-200">
