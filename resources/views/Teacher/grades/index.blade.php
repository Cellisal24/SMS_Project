    ...
    <div class="page-heading">
      <div class="page-heading-copy">
        ...
        <div>
          ...
          <p class="text-muted mb-0">Choose a class and subject to enter scores.</p>
        </div>
      </div>
    </div>
    ...
    <section class="panel mt-3">
      <div class="row g-3 align-items-end">
        <div class="col-12 col-md-4">
          <label class="form-label small">ថ្នាក់ & មុខវិជ្ជា / Class & Subject</label>
          <select id="combo_select" class="form-select">
            <option value="">-- Select --</option>
            @foreach ($combos as $combo)
              <option value="{{ $combo->class_id }}|{{ $combo->subject_id }}">
                {{ $combo->schoolClass->class_name ?? $combo->class_id }} — {{ $combo->subject->subject_name ?? $combo->subject_id }}
              </option>
            @endforeach
          </select>
        </div>
        ...
        <div class="col-12 col-md-4">
          ...
          <input list="semesterOptions" id="semester" class="form-control" placeholder="e.g. Semester 1 2026">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  loadBtn.addEventListener('click', function () {
    const comboValue = document.getElementById('combo_select').value;
    const semester = document.getElementById('semester').value.trim();

    if (!comboValue || !semester) {
      alert('Please select a class/subject and enter a semester first.');
      return;
    }

    const [classId, subjectId] = comboValue.split('|');

    fetch(`{{ route('teacher.grades.roster') }}?class_id=${classId}&subject_id=${subjectId}&semester=${encodeURIComponent(semester)}`)
      .then(res => res.json())
      .then(data => {
